...
def get_excel_summary_per_rut(pending_invoices, pending_movements, matches):
    ...
    for rut in ruts:
        ...
        matched_mov_amount = sum(rut_matches['mov_amount'])
    ...
    ruts_summary = pd.DataFrame(ruts_summary).sort_values(by='Facutras conciliadas %', ascending=False)
    with pd.ExcelWriter('Resumen matching clientes.xlsx') as writer:
        ruts_summary.to_excel(writer, sheet_name="Resumen", index=False)
